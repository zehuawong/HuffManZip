...
void huffmanZipByWord(string filename,string zipname)
{
	...
	string wordstr = "";
	string chstr = "";
	int wordsum = 0;	//单词总数（非种类数）
	string bufferInStr; //改进方法，一次性读取磁盘文件到内存中的buffstr
	...
	bufferInStr.assign(istreambuf_iterator<char>(infile), istreambuf_iterator<char>());
	do {
		for (int j = 0; j < bufferInStr.size(); j++)
		{
			ch = bufferInStr[j];
			if ((ch >= 'a'&&ch <= 'z') || (ch >= 'A'&&ch <= 'Z'))
				wordstr += ch;
	...
	} while (getline(infile, bufferInStr)); 
	//如果最后一个字符是字母
	if ((ch >= 'a'&&ch <= 'z') || (ch >= 'A'&&ch <= 'Z')) {
		codestr = huffmanWordMap[wordstr]->huffcode;
		for (int i = 0; i < codestr.size(); i++) {
...
{	
	...
	int wordsum = 0;
	fin.read((char*)&wordsum, sizeof(int)); //获取单词总数
	int wordNum = 0;
	fin.read((char*)&wordNum, sizeof(int)); //获取单词种类数
	string wordstr; int freq; char ch; int spaceflag=0;
	//cout << wordNum << "种单词" << endl;
	...
	if (root == NULL)
		return;
	makeHuffmanCode(root, ""); //生成各个单词的Huffman编码串
	//printLeaf(root);
	//开始解码
	char tempchar = 0; 
	HuffNode *tempNode = root; //初始指向根节点 //注意要防止最后一个字节编码读两次（用wordsum--控制）
		...
		for (int j = 0; j < bufferInStr.size(); j++)
		{
			...
			for (int i = sizeof(char) * 8 - 1; i >= 0; i--)
			{
				...
				if (tempNode->left == NULL) //找到叶节点则将单词写入文件
				{
					...
					//fout.write((char*)&wordstr, sizeof(wordstr));//注意： 用sizeof(string)是有错误的
					//无论你的string里放多长的字符串，它的sizeof()都是固定的
					//fout.write((char*)&wordstr, wordstr.size()); //这种方法也是错误的
					//fout << wordstr;
					bufferOutStr += wordstr;
					//cout <<wordstr;
...
int main(int argc, char *argv[])
{
	...
	{
		cout << "命令格式错误！" << endl;
		cout << "压缩命令格式应该为：huffmanZipByWord filename zipfilename" << endl;
		//cout << "解压缩命令格式应该为：unzipHuffFileByWord filename" << endl;
		exit(1);
	}
	...
	huffmanZipByWord(argv[1], argv[2]);
	//unzipHuffFileByWord(argv[1]);
	endTime = clock();
...
/*
	...
	//string filename = "files/傲慢+百年+麦田.txt";
	...
	cout << "用时"<<sumTime*1.0 /TEST_NUM/1000.0 << "s" << endl;
	system("pause");
	*/
	return 0;
}
